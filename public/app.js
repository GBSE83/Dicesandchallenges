...
if (!navigator.mediaDevices || !window.RTCPeerConnection) {
    alert('Tu navegador no soporta WebRTC. Actualízalo o usa Chrome/Firefox');
    throw new Error('Browser not supported');
}

// En la función initConnection() de connection.js
function initConnection() {
    if (!window.Peer) {
        showError('PeerJS no está cargado. Recarga la página');
        return;
    }
    // Resto del código...
}
...
const gameState = {
    ...
    playerStateHistory: [],
    diceMaxValue: 10, 
    allowDuplicateRolls: false,
    // Nuevas propiedades agregadas:
    syncSeed: null,         // Para sincronizar dados
    lastActionTimestamp: 0, // Para ordenar actualizaciones
    modalContent: ''        // HTML del modal de dados
};
...
function rollDice() {
    if (!rollNoChallengeCheckbox.checked && !gameState.gameStarted) return;

    // ========== INICIO DE MODIFICACIÓN ==========
    // Generar semilla compartida
    gameState.syncSeed = Date.now();
    Math.seedrandom(gameState.syncSeed);
    ...
    if (connectionState.role === 'host') {
        connectionState.guestConnections.forEach(conn => {
            conn.send({
                type: 'dice-roll',
                seed: gameState.syncSeed,
                players: JSON.parse(JSON.stringify(gameState.players)),
                challenges: JSON.parse(JSON.stringify(gameState.challenges))
            });
        });
    }
    // ========== FIN DE MODIFICACIÓN ==========

    // Resto de tu lógica original de rollDice...
    ...
    gameState.modalContent = modalDiceContainer.innerHTML;
...
// Función para recibir actualizaciones
function handleNetworkMessage(data) {
    if (data.type === 'dice-roll' && connectionState.role === 'guest') {
        // Sincronizar dados
        Math.seedrandom(data.seed);
        gameState.players = data.players;
        gameState.challenges = data.challenges;
        modalDiceContainer.innerHTML = gameState.modalContent;
        performGameRoll();
    }
...
// Sincronizar acciones críticas
const syncActions = ['addPlayer', 'addChallenge', 'startGame', 'undoLastRoll'];
