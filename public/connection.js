...
let connectionState = {
    ...
    peer: null,          // Instancia de PeerJS
    hostConnection: null,// Conexión del host (para guest)
    guestConnections: [],// Lista de conexiones (para host)
    isConnected: false,
    retryCount: 0,       // Intentos de reconexión
    lastPing: Date.now() // Para verificar conexión
};
...
if (connectionState.role === 'host') {
    setInterval(broadcastGameState, 2000); // Sync every 2 seconds
}
...
function handleNetworkMessage(data) {
    ...
    if (data.type === 'dice-roll') {
        window.handleNetworkMessage(data); // Usar la función de app.js
    }
}
...
setInterval(() => {
    ...
    if (Date.now() - connectionState.lastPing > 5000) {
        connectionState.isConnected = false;
    }
}, 3000);